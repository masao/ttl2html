...
RSpec.describe TTL2HTML::App do
  ...
  context "#output_html_files" do
    ...
    it "should respect subtitle_perclass settings" do
      @ttl2html = TTL2HTML::App.new(File.join(spec_base_dir, "example/example_subtitle_perclass.yml"))
      @ttl2html.load_turtle(File.join(spec_base_dir, "example/example_description.ttl"))
      @ttl2html.output_html_files
      cont = File.open("/tmp/html/b.html").read
      html = Capybara.string cont
    ...
    it "should generate ordering resources with schema:position for index.html" do
      @ttl2html = TTL2HTML::App.new(File.join(spec_base_dir, "example/example.yml"))
      @ttl2html.load_turtle(File.join(spec_base_dir, "example/example_top_schema_position.ttl"))
      @ttl2html.output_html_files
      expect(File).to exist "/tmp/html/index.html"
      cont = File.open("/tmp/html/index.html").read
      #puts cont
    ...
    it "should generate ordering resources with schema:position (https) for index.html" do
      @ttl2html = TTL2HTML::App.new(File.join(spec_base_dir, "example/example.yml"))
      @ttl2html.load_turtle(File.join(spec_base_dir, "example/example_top_schema_position2.ttl"))
      @ttl2html.output_html_files
      expect(File).to exist "/tmp/html/index.html"
      cont = File.open("/tmp/html/index.html").read
      #puts cont
    ...
    it "should generate versions information" do
      ...
      expect(html).to have_css("h2#versions + dl + p a[href='about.html#versions']")
    end
    ...
    it "should add class=url for the name and example columns in shapes table" do
      ...
      expect(html).to have_css("table td.url", text: "mailto:alice@example.org")
      expect(html).to have_css("table td.url", text: "https://example.org/b")
    end
    it "should output datatypes on the shape table" do
      @ttl2html = TTL2HTML::App.new(File.join(spec_base_dir, "example/example.yml"))
      @ttl2html.load_turtle(File.join(spec_base_dir, "example/example_shape_datatype.ttl"))
      @ttl2html.output_html_files
      cont = open("/tmp/html/about.html"){|io| io.read }
      html = Capybara.string cont
      #puts cont
    ...
    it "should link to custom css_file" do
      @ttl2html = TTL2HTML::App.new(File.join(spec_base_dir, "example/css_file_single.yml"))
      @ttl2html.load_turtle(File.join(spec_base_dir, "example/example.ttl"))
      @ttl2html.output_html_files
      cont = open("/tmp/html/a/index.html"){|io| io.read }
      html = Capybara.string cont
      expect(html).to have_css("link[href='../custom.css']", visible: false)
      cont = open("/tmp/html/b.html"){|io| io.read }
      html = Capybara.string cont
      expect(html).to have_css("link[href='custom.css']", visible: false)
    end
    ...
    it "should output itemtype attr for Microdata markup" do
      @ttl2html = TTL2HTML::App.new(File.join(spec_base_dir, "example", "example.yml"))
      @ttl2html.load_turtle(File.join(spec_base_dir, "example", "example.ttl"))
      @ttl2html.output_html_files
      cont = open("/tmp/html/a/index.html"){|io| io.read }
